...
Scope {
    ...
    function setValue(key, value): void {
        ...
        Quickshell.execDetached(["bash", root.helper, "set", key, stringValue]);
    ...
    function setNightLightAuto(value): void {
        root.nightLightAuto = value;
        root.setBool("NIGHT_LIGHT_AUTO", value);
    }

    function setNightLightManual(value): void {
        root.nightLightManual = value;
        root.setBool("NIGHT_LIGHT_MANUAL", value);
    }

    function setNightLightTemperature(value): void {
        const temperature = Math.max(1000, Math.min(20000, Math.round(value)));
        root.nightLightTemperature = temperature;
        root.setValue("NIGHT_LIGHT_TEMPERATURE", temperature);
    }
    ...
    function applyNightLightSchedule(fromMinutes, untilMinutes): void {
        root.nightLightFromMinutes = Math.max(0, Math.min(1439, fromMinutes));
        root.nightLightUntilMinutes = Math.max(0, Math.min(1439, untilMinutes));
        root.setValue("NIGHT_LIGHT_FROM", root.nightLightFromMinutes);
        root.setValue("NIGHT_LIGHT_UNTIL", root.nightLightUntilMinutes);
        root.statusText = "Applied";
        statusTimer.restart();
    }
    ...
    function switchWorkspace(number): void {
        Quickshell.execDetached(["bash", root.helper, "workspace", String(number)]);
    }
    ...
    function openWelcome(): void {
        Quickshell.execDetached(["qs", "ipc", "call", "welcome", "open"]);
    }

    function clearNotifications(): void {
        Quickshell.execDetached(["qs", "ipc", "call", "notifications", "clear"]);
    }

    function openMonitorSettings(): void {
        Quickshell.execDetached(["qs", "ipc", "call", "controlcenter", "monitor"]);
    }

    function openBrowser(): void {
        Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/hypr/settings/browser.sh"]);
    }

    function openFileManager(): void {
        Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/hypr/settings/filemanager.sh"]);
    }

    function openCalculator(): void {
        Quickshell.execDetached(["bash", Quickshell.env("HOME") + "/.config/hypr/settings/calculator.sh"]);
    }
    ...
    function changeAudioVolume(deltaPercent): void {
        ...
        Quickshell.execDetached(["wpctl", "set-volume", "-l", "1.5", "@DEFAULT_AUDIO_SINK@", deltaPercent < 0 ? step + "-" : step + "+"]);
    ...
    function setAudioMuted(value): void {
        ...
        Quickshell.execDetached(["wpctl", "set-mute", "@DEFAULT_AUDIO_SINK@", value ? "1" : "0"]);
            ...
            onStreamFinished: {
                ...
                for (let line of lines) {
                    ...
                    if (key === "WORKSPACE_COUNT")
                        ...
                        root.nightLightAuto = root.boolValue(value);
                    else if (key === "NIGHT_LIGHT_MANUAL")
                        root.nightLightManual = root.boolValue(value);
                    else if (key === "NIGHT_LIGHT_FROM")
                        root.nightLightFromMinutes = Math.max(0, Math.min(1439, parseInt(value)));
                    else if (key === "NIGHT_LIGHT_UNTIL")
                        root.nightLightUntilMinutes = Math.max(0, Math.min(1439, parseInt(value)));
                    else if (key === "NIGHT_LIGHT_TEMPERATURE")
                        root.nightLightTemperature = Math.max(1000, Math.min(20000, parseInt(value)));
                }
            }
        }
    }
